...
def environment_validation_fails(env: CompilerEnv) -> bool:
    ...
    logging.debug(str(validation_result).split("\n")[0][:120])
    return not validation_result.okay()


def _apply_and_test(env, actions, hypothesis, flakiness) -> bool:
    """Run specifiec actions on environment and return whether hypothesis holds."""
...
def bisect_trajectory(
    ...
    :raises MinimizationError: If the minimization fails, or if the hypothesis
        does not hold on the initial trajectory.
    """
...
def random_minimization(
    ...
    while len(actions) > 5 and num_steps < max_num_steps:
        num_steps += 1
        num_to_remove = int(ceil(len(actions) * discard_ratio))
        candidate_actions = actions.copy()
        for _ in range(num_to_remove):
            candidate_actions.remove(random.choice(candidate_actions))
...
def minimize_trajectory_iteratively(
    ...
    if not all_actions:
        raise ValueError("No actions in the given state.")
